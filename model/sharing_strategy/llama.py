...
def sharing_strategy(cfg, model):
    ...
    if cfg.recursive.num_recursion == 1:
        ...
        for layer_idx in range(base_depth): # Here the sharing strategy is applied
            ...
            # Example for middle_cycle with 29 layers and num_recursion=3:
            #   - layer 0 is kept unique
            #   - layers 1..27 form 9 shared groups across 3 recursions:
            #       [1, 10, 19], [2, 11, 20], ..., [9, 18, 27]
            #   - layer 28 is kept unique
            # This gives 11 unique layers in total: 1 first + 9 shared middle + 1 last.
